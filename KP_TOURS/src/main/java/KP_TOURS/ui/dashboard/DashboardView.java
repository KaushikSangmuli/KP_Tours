...
// Also added: Ctrl+F to focus global search
    ...
    private static void initializeTable() {
        ...
        tripTable.setItems(TripCacheManager.getTripCache());
    }

    public static void loadTripsForDate(LocalDate date) {
        selectedDateLabel.setText(
                "Trips for " + date.format(DateTimeFormatter.ofPattern("dd MMM yyyy")));
        FilteredList<Trip> filtered = new FilteredList<>(
                TripCacheManager.getTripCache(),
                trip -> trip.getTripDate() != null && trip.getTripDate().equals(date)
        );
